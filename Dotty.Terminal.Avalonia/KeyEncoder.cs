...
public static class KeyEncoder
{
    ...
    public static TerminalKey MapKey(Key key)
    {
        var value = (int)key;
        var mapped = TerminalKeyMapping.MapAlphabeticKey(value, (int)Key.A);
        if (mapped != TerminalKey.None)
            return mapped;

        mapped = TerminalKeyMapping.MapDigitKey(value, (int)Key.D0);
        if (mapped != TerminalKey.None)
            return mapped;

        mapped = TerminalKeyMapping.MapNumberPadDigitKey(value, (int)Key.NumPad0);
        if (mapped != TerminalKey.None)
            return mapped;
